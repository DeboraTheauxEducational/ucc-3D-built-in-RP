    ...
    SubShader
    {
       Tags { "RenderType"="Transparent" "Queue"="Transparent" "ForceNoShadowCasting"="True"}
        ...
        void surf (Input IN, inout SurfaceOutputStandard o) {
            ...
            // Moving specs
           float timeFactor = _Time.y * 1;
            float wave = sin(IN.uv_Specs.x * 1 + timeFactor) * 0.1;
            float2 specCoords = IN.uv_Specs + _SpecDirection * timeFactor + float2(wave, wave);
            col.rgb = lerp(col.rgb, tex2D(_Specs, specCoords).rgb * _SpecColor.rgb, _SpecColor.a);
